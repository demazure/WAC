...
{
  if (reportDebug())
    cout << "EposEventReader::EposEventReader(...) No ops" << endl;
}
...
{
  if (reportDebug())
    cout << "EposEventReader::initialize() Started" << endl;
  ...
  if (reportInfo())
    cout << "EposEventReader::initialize() nEntries: " << nentries << endl;
  if (nentries < 1) {
    if (reportError())
      cout << "EposEventReader::initialize() no data found. Abort." << endl;
    postTaskFatal();
    return;
  }
  nbytes = 0;
  nb = 0;
  if (reportDebug())
    cout << "EposEventReader::initialize() Completed" << endl;
}
...
{
  if (reportDebug())
    cout << "EposEventReader::reset() Started" << endl;
  event->reset();
  Particle::getFactory()->reset();
  if (reportDebug())
    cout << "EposEventReader::reset() Completed" << endl;
}
...
{
  if (reportDebug())
    cout << "EposEventReader::execute() Started" << endl;

  if (!fChain) {
    if (reportFatal())
      cout << " EposEventReader::execute() no TChain available" << endl;
    postTaskFatal();
    return;
  }
  ...
  if (mult > arraySize) {
    if (reportError())
      cout << "EposEventReader::execute() n particles is " << mult << " and exceeds capacity " << arraySize << endl;
    postTaskError();
    return;
  }
  if (reportDebug())
    cout << "EposEventReader::execute() Impact par: " << impact << "  " << mult << endl;
  ...
  if (reportDebug())
    cout << "EposEventReader::execute() No of accepted Particles : " << particleAccepted << endl;
  if (reportDebug())
    cout << "EposEventReader::execute() event completed!" << endl;
}
