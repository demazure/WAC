...
Bool_t EventPool::EventMatchesBin(Int_t mult, Double_t zvtx, Double_t psi, Double_t pt) const
{
  return EventMatchesBin((Double_t) mult, zvtx, psi, pt);
}

Bool_t EventPool::EventMatchesBin(Double_t mult, Double_t zvtx, Double_t psi, Double_t pt) const
{
...
Int_t EventPool::NTracksInPool() const
{
...
Int_t EventPool::SetEventMultRange(Double_t multMin, Double_t multMax)
{
  fMultMin = multMin;
  fMultMax = multMax;
  return 0;
...
Int_t EventPool::SetEventZvtxRange(Double_t zvtxMin, Double_t zvtxMax)
{
  fZvtxMin = zvtxMin;
  fZvtxMax = zvtxMax;
  return 0;
...
Int_t EventPool::SetEventPsiRange(Double_t psiMin, Double_t psiMax)
{
  fPsiMin = psiMin;
  fPsiMax = psiMax;
  return 0;
...
Int_t EventPool::SetEventPtRange(Double_t ptMin, Double_t ptMax)
{
  fPtMin = ptMin;
  fPtMax = ptMax;
  return 0;
...
Int_t EventPool::GlobalEventIndex(Int_t j) const
{
  /// Index returned from passing local pool event index.

  if (j < 0 || j >= (Int_t)fEventIndex.size()) {
    cout << "ERROR in EventPool::GlobalEventIndex(): "
...
Int_t EventPool::UpdatePool(TObjArray* trk)
{
  ...
  if(fLockFlag)
  {
    Fatal("EventPool::UpdatePool", "Tried to fill a locked EventPool.");
    return fEvents.size();
  }
...
Long64_t EventPool::Merge(TCollection* hlist)
{
  if (!hlist)
  	return 0;
  ...
  while ((tmpObj = static_cast<EventPool*>(objIter()))) {
    // Update this pool (it won't get fuller than demanded)
    for (uint i = 0; i < tmpObj->fEvents.size(); i++)
...
TObject* EventPool::GetRandomTrack() const
{
...
TObjArray* EventPool::GetEvent(Int_t i) const
{
  if (i<0 || i>=(Int_t)fEvents.size()) {
    cout << "EventPool::GetEvent("
...
TObjArray* EventPool::GetRandomEvent() const
{
  UInt_t ranEvt = gRandom->Integer(fEvents.size());
  TObjArray *tca = fEvents.at(ranEvt);
  return tca;
...
ClassImp(EventPoolManager)
  ...
  EventPoolManager::EventPoolManager(Int_t depth, Int_t minNTracks,
                                     Int_t nMultBins, Double_t* multbins,
                                     Int_t nZvtxBins, Double_t* zvtxbins) : fDebug(0), fNMultBins(0), fNZvtxBins(0), fNPsiBins(0), fNPtBins(0), fMultBins(), fZvtxBins(), fPsiBins(), fPtBins(), fEvPool(0), fTargetTrackDepth(minNTracks)
{
  // Constructor.
  ...
  cout << "EventPoolManager initialized." << endl;
}

EventPoolManager::EventPoolManager(Int_t depth, Int_t minNTracks,
                                   Int_t nMultBins, Double_t* multbins,
                                   Int_t nZvtxBins, Double_t* zvtxbins,
                                   Int_t nPsiBins, Double_t* psibins) : fDebug(0), fNMultBins(0), fNZvtxBins(0), fNPsiBins(0), fNPtBins(0), fMultBins(), fZvtxBins(), fPsiBins(), fPtBins(), fEvPool(0), fTargetTrackDepth(minNTracks)
{
  // Constructor.
  // without pt bins
  Int_t nPtBins = 1;
...
EventPoolManager::EventPoolManager(Int_t depth, Int_t minNTracks,
                                   ...
                                   Int_t nPtBins, Double_t* ptbins) : fDebug(0), fNMultBins(0), fNZvtxBins(0), fNPsiBins(0), fNPtBins(0), fMultBins(), fZvtxBins(), fPsiBins(), fPtBins(), fEvPool(0), fTargetTrackDepth(minNTracks)
{
  // Constructor.
...
EventPoolManager::EventPoolManager(Int_t depth, Int_t minNTracks, const char* binning) : fDebug(0), fNMultBins(0), fNZvtxBins(0), fNPsiBins(0), fNPtBins(0), fMultBins(), fZvtxBins(), fPsiBins(), fPtBins(), fEvPool(0), fTargetTrackDepth(minNTracks)
{
...
{
  // Assign EventPoolManager members. (with Event plane + pt)
    ...
    for (Int_t iZ=0; iZ<nZvtxBins; iZ++) {
      for (Int_t iP=0; iP<nPsiBins; iP++) {
        for (Int_t iPt=0; iPt<nPtBins; iPt++) {
          ...
          fEvPool.push_back(new EventPool(depth,
...
Long64_t EventPoolManager::Merge(TCollection* hlist)
{
  if (!hlist)
    return 0;

  EventPoolManager* tmpObj = 0;
  TIter objIter(hlist);

  // Iterate through all objects to be merged
  while ((tmpObj = static_cast<EventPoolManager*>(objIter()))) {
      ...
      for(Int_t j = 0; j<GetNumberOfZVtxBins(); j++)
        for(Int_t k = 0; k<GetNumberOfPsiBins(); k++)
          for(Int_t l = 0; l<GetNumberOfPtBins(); l++)
          {
            ...
            EventPool* objPool = tmpObj->GetEventPool(i, j, k, l);
            EventPool* pool = GetEventPool(i, j, k, l);
...
void EventPoolManager::SetTargetValues(Int_t trackDepth, Float_t fraction, Int_t events)
{
...
void EventPoolManager::ClearPools()
{
...
void EventPoolManager::ClearPools(Double_t minCent, Double_t maxCent, Double_t minZvtx, Double_t maxZvtx, Double_t minPsi, Double_t maxPsi, Double_t minPt, Double_t maxPt)
{
  ...
  for(Int_t i = 0; i<GetNumberOfMultBins(); i++)
    for(Int_t j = 0; j<GetNumberOfZVtxBins(); j++)
      for(Int_t k = 0; k<GetNumberOfPsiBins(); k++)
        for(Int_t l = 0; l<GetNumberOfPtBins(); l++)
        {
          EventPool* pool = GetEventPool(i, j, 0, l);
...
void EventPoolManager::SetSaveFlag(Double_t minCent, Double_t maxCent, Double_t minZvtx, Double_t maxZvtx, Double_t minPsi, Double_t maxPsi, Double_t minPt, Double_t maxPt)
{
    ...
    for(Int_t j = 0; j<GetNumberOfZVtxBins(); j++)
      for(Int_t k = 0; k<GetNumberOfPsiBins(); k++)
        for(Int_t l = 0; l<GetNumberOfPtBins(); l++)
        {
          ...
          {
            if(pool->GetLockFlag())
              Warning("EventPoolManager::SetSaveFlag", "A pool that was already imported is flagged to be saved. Is this really intended?");
            pool->SetSaveFlag(kTRUE);
          }
        }  
}

void EventPoolManager::Validate()
{
  std::cout << "############## EventPoolManager ##############\n";
  std::cout << "== Binning ==\n";
  TString tmpStr = "cent/mult: ";
  for (uint i = 0; i < fMultBins.size(); i++)
    tmpStr += Form("%4.3f ", fMultBins[i]);
  tmpStr += " vertex: ";
  for (uint i = 0; i < fZvtxBins.size(); i++)
    tmpStr += Form("%4.3f ", fZvtxBins[i]);
  tmpStr += " psi: ";
  for (uint i = 0; i < fPsiBins.size(); i++)
    tmpStr += Form("%4.3f ", fPsiBins[i]);
  tmpStr += " pt: ";
  for (uint i = 0; i < fPtBins.size(); i++)
  ...
  for (Int_t iM=0; iM<fNMultBins; iM++)
    for (Int_t iZ=0; iZ<fNZvtxBins; iZ++)
      for (Int_t iP=0; iP<fNPsiBins; iP++)
        for (Int_t iPt=0; iPt<fNPtBins; iPt++) 
        {
          EventPool* pool = GetEventPool(iM, iZ, iP, iPt);
          if(!pool)
            Fatal("EventPoolManager::Validate", "Pool (%i,%i,%i,%i) is not correctly initialized!", iM, iZ, iP, iPt);
          if(pool->GetLockFlag())
...
EventPool* EventPoolManager::GetEventPool(Int_t iMult, Int_t iZvtx, Int_t iPsi, Int_t iPt) const
{
  if (iMult < 0 || iMult >= fNMultBins) 
  {
    Error("EventPoolManager::GetEventPool", "Mult bin %i exceeds maximum of %i", iMult, fNMultBins);
    return 0x0;
  }
  if (iZvtx < 0 || iZvtx >= fNZvtxBins) 
  {
    Error("EventPoolManager::GetEventPool", "Zvtx bin %i exceeds maximum of %i", iZvtx, fNZvtxBins);
    return 0x0;
  }
  if (iPsi < 0 || iPsi >= fNPsiBins) 
  {
    Error("EventPoolManager::GetEventPool", "Psi bin %i exceeds maximum of %i", iPsi, fNPsiBins);
    return 0x0;
  }
  if (iPt < 0 || iPt >= fNPtBins) 
  {
    Error("EventPoolManager::GetEventPool", "Pt bin %i exceeds maximum of %i", iPt, fNPtBins);
    return 0x0;
  }
  ...
  if(fEvPool.at(GetBinIndex(iMult, iZvtx, iPsi, iPt)))
    ...
    Fatal("EventPoolManager::GetEventPool", "Pool does not exist");

  return 0;
}

EventPool* EventPoolManager::GetEventPool(Int_t centVal, Double_t zVtxVal, Double_t psiVal, Int_t iPt) const
{
  return GetEventPool((Double_t)centVal, zVtxVal, psiVal, iPt);
}

EventPool* EventPoolManager::GetEventPool(Double_t centVal, Double_t zVtxVal, Double_t psiVal, Int_t iPt) const
{
...
Int_t EventPoolManager::UpdatePools(TObjArray* trk)
{
...
Double_t* EventPoolManager::GetBinning(const char* configuration, const char* tag, Int_t& nBins) const
{
