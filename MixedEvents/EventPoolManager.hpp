...
class EventPool : public TObject
{
 public:
  ...
  EventPool(Int_t d, Double_t multMin, Double_t multMax,
...
protected:
  ...
  ClassDef(EventPool, 4) // Event pool class
};
...
public:
 EventPoolManager()
   : fDebug(0),
     ...
     fEvPool(0),
     fTargetTrackDepth(0) {}
 EventPoolManager(Int_t maxEvts, Int_t minNTracks,
 ...
 EventPoolManager(Int_t maxEvts, Int_t minNTracks, const char* binning);

 ~EventPoolManager() { ; }
 Long64_t Merge(TCollection* hlist);

 // First uses bin indices, second uses the variables themselves.
 EventPool* GetEventPool(Int_t iMult, Int_t iZvtx, Int_t iPsi = 0, Int_t iPt = 0) const;
 EventPool* GetEventPool(Int_t centVal, Double_t zvtxVal, Double_t psiVal = 0., Int_t iPt = 0) const;
 EventPool* GetEventPool(Double_t centVal, Double_t zvtxVal, Double_t psiVal = 0., Int_t iPt = 0) const;
...
protected:
 ...
 std::vector<EventPool*> fEvPool; ///< pool in bins of [fNMultBin][fNZvtxBin][fNPsiBin][fNPtBins]
