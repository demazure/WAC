...
ClassImp(EnhancedGraph)
  ...
  EnhancedGraph::EnhancedGraph(const TString& graphName,
                               ...
                               double maxY, int n, double* x, double* y,
                               double* ex, double* ey, int opt)
  : TH1D(graphName, graphName, 1, minX, maxX), nPoints(n), xPtr(x), exPtr(ex), yPtr(y), eyPtr(ey), ySysMPtr(0), ySysPPtr(0), yM(0), yP(0), sysAbsolute(false), option(opt)
{
  SetXTitle(xTitle);
  SetYTitle(yTitle);
  SetMinimum(minY);
