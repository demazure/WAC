...
ClassImp(NuDynTask)
  ...
  // CTOR
  //////////////////////////////////////////////////////////////
  NuDynTask::NuDynTask(const TString& name, TaskConfiguration* configuration,
                       Event* event, EventFilter* ef, ParticleFilter* pf1,
                       ParticleFilter* pf2, ParticleFilter* pf3,
                       ParticleFilter* pf4)
  : Task(name, configuration, event), nuDynHistos(NULL), nuDynDerivedHistos(NULL), eventFilter(ef), particleFilter1(pf1), particleFilter2(pf2), particleFilter3(pf3), particleFilter4(pf4), partName1("U"), partName2("U"), partName3("U"), partName4("U")
{
  if (reportDebug())
    cout << "NuDynTask::CTOR(...) Started." << endl;

  if (!eventFilter) {
    if (reportError())
      cout << "NuDynTask::CTOR(...) eventFilter is null pointer." << endl;
    postTaskError();
    return;
  }
  ...
  if (!particleFilter1) {
    if (reportError())
      cout << "NuDynTask::CTOR(...) particleFilter1 is null pointer." << endl;
    postTaskError();
    return;
  }
  if (!particleFilter2) {
    if (reportError())
      cout << "NuDynTask::CTOR(...) particleFilter2 is null pointer." << endl;
    postTaskError();
    return;
  }
  if (!particleFilter3) {
    if (reportError())
      cout << "NuDynTask::CTOR(...) particleFilter3 is null pointer." << endl;
    postTaskError();
    return;
  }
  if (!particleFilter4) {
    if (reportError())
      cout << "NuDynTask::CTOR(...) particleFilter4 is null pointer." << endl;
    postTaskError();
    return;
  }
...
{
  if (reportDebug())
    cout << "NuDynTask::DTOR(...) Started" << endl;
  if (nuDynHistos != NULL)
    delete nuDynHistos;
  if (nuDynDerivedHistos != NULL)
    delete nuDynDerivedHistos;
  if (reportDebug())
    cout << "NuDynTask::DTOR(...) Completed" << endl;
}

void NuDynTask::createHistograms()
{
  if (reportDebug())
    cout << "NuDynTask::initialize(...) started" << endl;
  ...
  if (reportDebug())
    cout << "NuDynTask::createHistograms(...) completed" << endl;
}
...
{
  if (reportDebug())
    cout << "NuDynTask::loadHistograms(...) Starting." << endl;
  ...
  if (reportDebug())
    cout << "NuDynTask::loadHistograms(...) Completed." << endl;
}
...
{
  if (reportDebug())
    cout << "NuDynTask::saveHistograms(...) Saving Event histograms to file." << endl;
  if (!outputFile) {
    if (reportError())
      cout << "NuDynTask::saveHistograms(...) outputFile is a null  pointer." << endl;
    postTaskError();
    return;
  }
  outputFile->cd();
  ...
  if (reportDebug())
    cout << "NuDynTask::saveHistograms(...) Completed." << endl;
}
...
void NuDynTask::execute()
{
  ...
  if (event != NULL) {
    if (reportDebug())
      cout << "NuDynTask::execute(...) analyzing " << event->nParticles << " particles" << endl;
  } else {
    if (reportError())
      cout << "NuDynTask::execute(...) event pointer is NULL. Abort." << endl;
    postTaskError();
    return;
  }
  ...
  if (!ac) {
    if (reportError())
      cout << "NuDynTask::execute(...) analysisConfiguration null pointer" << endl;
    postTaskError();
    return;
  }
  ...
  eventsProcessed++;
  if (reportDebug())
    cout << "NuDyn::execute() Completed" << endl;
}

void NuDynTask::calculateDerivedHistograms()
{
  if (reportDebug())
    cout << "NuDynTask::calculateDerivedHistograms() Starting" << endl;
  nuDynDerivedHistos->calculateDerivedHistograms(nuDynHistos);
  if (reportDebug())
    cout << "NuDynTask::calculateDerivedHistograms() Completed" << endl;
}
...
{
  if (reportDebug())
    cout << "NuDynTask::scaleHistograms(..) Scale all primary histograms by " << factor << endl;
  nuDynHistos->scale(factor);
  if (reportDebug())
    cout << "NuDynTask::scale(..) Completed" << endl;
}
