...
void NuDynDerivedHistos::loadHistograms(TFile* inputFile)
{
  if (!inputFile) {
    if (reportFatal())
      cout << "-Fatal- Attempting to load NuDynDerivedHistos from an invalid file pointer" << endl;
    return;
  }
  AnalysisConfiguration& ac = *getConfiguration();
  TString bn = getHistoBaseName();
...
void NuDynDerivedHistos::calculateDerivedHistograms(NuDynHistos* source)
{
  ...
  if (!source) {
    if (reportFatal())
      cout << "-Fatal- NuDynDerivedHistos::calculateDerivedHistograms(NuDynHistos * source) Source is null." << endl;
    return;
  }
  ...
  for (int i1 = 0; i1 < nPart; i1++) {
    for (int i2 = i1; i2 < nPart; i2++) {
      ...
      if (ac.nuDynVsCent)
        calculateF2R2(source->h_f1_vsCent[i1], source->h_f1_vsCent[i2], source->h_f2_vsCent[index12], h_F2_vsCent[index12], h_R2_vsCent[index12]);
      for (int i3 = i2; i3 < nPart; i3++) {
        ...
        if (ac.nuDynVsCent)
          calculateF3R3(source->h_f1_vsCent[i1], source->h_f1_vsCent[i2], source->h_f1_vsCent[i3],
        ...
        for (int i4 = i3; i4 < nPart; i4++) {
          ...
          if (ac.nuDynVsCent)
            calculateF4R4(source->h_f1_vsCent[i1], source->h_f1_vsCent[i2], source->h_f1_vsCent[i3], source->h_f1_vsCent[i4],
