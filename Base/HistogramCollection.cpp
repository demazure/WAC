...
{
  if (reportDebug())
    cout << "HistogramCollection::plotHistograms(...) collection: " << collectionName << " started  w/ nHistograms =" << nHistograms << endl;
  ...
  for (int iHisto = 0; iHisto < nHistograms; iHisto++) {
    ...
    if (isPlotted[iHisto]) {
      if (h->IsA() == TH1::Class() ||
          ...
          h->IsA() == TH1D::Class()) {
        if (reportDebug())
          cout << "-INFO- HistogramCollection::plotHistograms(...) Plotting 1D histo #" << iHisto << " named " << h->GetTitle() << endl;
        ...
        if (nx * ny <= 10000) {
          if (reportDebug())
            cout << "-INFO- HistogramCollection::plotHistograms(...) Plotting 2D histo #" << iHisto << " named " << h->GetTitle() << endl;
          canvas = canvasCollection.createCanvas(canvasName, cc2d, 30);
          setHistoProperties(h, gc2D);
          h->Draw(gc2D.plotOption);
        } else {
          if (reportDebug())
            cout << "-INFO- HistogramCollection::plotHistograms(...) Skipping 2D histo #" << iHisto << " named " << h->GetTitle() << endl;
        }
      } else if (h->IsA() == TH3::Class()) {
        if (reportDebug())
          cout << "-INFO- plotHistograms(...) Skipping 3D histo called " << h->GetTitle() << endl;
      } else {
        if (reportDebug())
          cout << "-INFO- plotHistograms(...) Skipping histo #" << iHisto << " of unfamiliar type named " << h->GetTitle() << endl;
      }
      ...
      if (reportDebug())
        cout << "-INFO- plotHistograms(...) Skipping histo #" << iHisto << " named " << h->GetTitle() << endl;
    }
  }
  if (reportDebug())
    cout << "HistogramCollection::plotHistograms(...) completed." << endl;
}
...
void HistogramCollection::setHistoProperties(TH2* h, const GraphConfiguration& graphConfiguration)
{
  if (reportDebug())
    cout << "----------------------------------------------------" << endl;
  if (reportDebug())
    cout << "Setting properties of 2D histo: " << h->GetTitle() << endl;
  h->SetLineColor(graphConfiguration.lineColor);
  h->SetLineStyle(graphConfiguration.lineStyle);
  h->SetLineWidth(graphConfiguration.lineWidth);
  TAxis* xAxis = (TAxis*)h->GetXaxis();
