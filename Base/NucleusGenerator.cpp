...
ClassImp(NucleusGenerator)
  ...
  : generatorType(_generatorType), generatorName(_generatorName), nR(_nR), minR(_minR), maxR(_maxR), parA(_parA), parB(_parB), parC(_parC), rDensity(0), rProfile(0), rProfileGen(0)
{
  initialize();
}
...
void NucleusGenerator::initialize()
{
  ...
  switch (generatorType) {
    case Uniform:
      generatorTypeName = "_Uniform";
      break;
    case WoodsSaxon:
      generatorTypeName = "_WoodsSaxon";
      break;
    case Exponential:
      generatorTypeName = "_Exponential";
      break;
    case Gaussian:
      generatorTypeName = "_Gaussian";
      break;
    case DoubleGaussian:
      generatorTypeName = "_DoubleGaussian";
      break;
  }
