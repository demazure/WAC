...
void StatStudyHistograms::loadHistograms(TFile* inputFile)
{
  if (!inputFile) {
    if (reportError())
      cout << "StatStudyHistograms::loadHistograms(...) inputFile is a null pointer" << endl;
    return;
  }
...
void StatStudyHistograms::saveHistograms(TFile* outputFile)
{
  if (!outputFile) {
    if (reportError())
      cout << "StatStudyHistograms::loadHistograms(...) outputFile is a null pointer" << endl;
    return;
  }
  outputFile->cd();
...
void StatStudyHistograms::calculateRatio(TH1* h1, TH1* h2, TH1* ratio, TH1* ratioOptB)
{
  ...
  if (ratioOptB)
    ratioOptB->Divide(h1, h2, 1.0, 1.0, "B");
...
void StatStudyHistograms::fillMeanErrStd(TH1* h, TH1* eh, int n)
{
  ...
  for (int i = 2; i <= n; i++) {
    ...
    eh->SetBinContent(i, std / fabs(mean));
    eh->SetBinError(i, 0);
  }
  ...
  cout << "           std:" << std << endl;
  h->SetBinContent(n + 5, mean);
  h->SetBinError(n + 5, typicalError);
  h->SetBinContent(n + 10, mean);
  h->SetBinError(n + 10, errorOnMean);
  h->SetBinContent(n + 15, mean);
  h->SetBinError(n + 15, std);
}
