...
/**
 ...
 Class defining RapidityGenerator
 */
...
ClassImp(RapidityGenerator)
  ...
  RapidityGenerator::RapidityGenerator(int opt, int pair, double max,
                                       double s, double minSep)
  : option(opt), pairMode(pair), maxRapidity(max), scale(s), minimumSeparation(minSep), nStored(0), pairRapidity(0), saveRapidity(0), rank(0)
{
  r = new TRandom();
}
