...
profile(nullptr)
{
  if (reportDebug()) cout << "GeneralizedGaussianProfile::CTOR() No Ops" << endl;
}
...
{
  if (reportDebug())  cout << "GeneralizedGaussianProfile::DTOR() Started." << endl;
  if (profile != nullptr) delete profile;
  profile = nullptr;
  if (reportDebug())  cout << "GeneralizedGaussianProfile::DTOR() Completed." << endl;
}
...
{
  if (reportDebug()) cout << "GeneralizedGaussianProfile::initialize() Started." << endl;
  ...
  if (reportDebug()) cout << "GeneralizedGaussianProfile::initialize() Complted." << endl;
}
...
{
  if (reportDebug()) cout << "GeneralizedGaussianProfile::process() Begin()" << endl;
  if (event != nullptr)
    {
    if (reportInfo()) cout << "GeneralizedGaussianProfile::process() processing " << event->nParticles << " particles" << endl;
    ...
    {
    if (reportError()) cout << "GeneralizedGaussianProfile::analyze(Event * event) Given event pointer is null." << endl;
    }
  if (reportDebug()) cout << "GeneralizedGaussianProfile::process() Begin()" << endl;
}
