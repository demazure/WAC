...
ClassImp(NuDynHistos)
  ...
  NuDynHistos::NuDynHistos(const TString& name, int* identicalUsed,
...
void NuDynHistos::loadHistograms(TFile* inputFile)
{
  ...
  if (ac.nuDynVsMult)
    h_events_vsMult = loadH1(inputFile, bn + TString("Nevents_vsMult"), ac.nBins_mult, ac.min_mult, ac.max_mult, "mult", "n_{Events}");
  if (ac.nuDynVsCent)
    h_events_vsCent = loadH1(inputFile, bn + TString("Nevents_vsCent"), ac.nBins_cent, ac.min_cent, ac.max_cent, "cent", "n_{Events}");
...
void NuDynHistos::fill(double mult, double cent, double* n, double weight)
{
  ...
  for (int i1 = 0; i1 < nPart; i1++) {
    ...
    if (ac.nuDynVsCent)
      h_f1_vsCent[i1]->Fill(cent, fill, weight);
    ...
    for (int i2 = i1; i2 < nPart; i2++) {
      ...
      if (ac.nuDynVsCent)
        h_f2_vsCent[index12]->Fill(cent, fill, weight);
      for (int i3 = i2; i3 < nPart; i3++) {
        int index123 = index3(i1, i2, i3);
        ...
        if (ac.nuDynVsCent)
          h_f3_vsCent[index123]->Fill(cent, fill, weight);
        for (int i4 = i3; i4 < nPart; i4++) {
          int index1234 = index4(i1, i2, i3, i4);
          ...
          if (ac.nuDynVsCent)
            h_f4_vsCent[index1234]->Fill(cent, fill, weight);
        }
      }
    }
  }
