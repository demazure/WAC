...
ClassImp(CollisionGeometryHistograms)
  ...
  CollisionGeometryHistograms::CollisionGeometryHistograms(
    ...
    CollisionGeometryConfiguration* _configuration, LogLevel debugLevel)
...
void CollisionGeometryHistograms::loadHistograms(TFile* inputFile)
{
  if (!inputFile) {
    if (reportFatal())
      cout << "-Fatal- Attempting to load CollisionGeometryHistograms from an invalid file pointer" << endl;
    return;
  }
  CollisionGeometryConfiguration& ac = *configuration;
  TString bn = ac.histoBaseName;
...
void CollisionGeometryHistograms::fill(CollisionGeometry* collisionGeometry, double weight)
{
  ...
  double counts = 0.0;
  double momX = 0.0;
  double momX2 = 0.0; // double momX3 = 0.0; double momX4 = 0.0; double momX6 = 0.0; double momX8 = 0.0;
  double momY = 0.0;
  double momY2 = 0.0; // double momY3 = 0.0; double momY4 = 0.0; double momY6 = 0.0; double momY8 = 0.0;
  double momXY = 0.0;
  for (int iBinary = 0; iBinary < nBinary; iBinary++) {
    x = collisionGeometry->x[iBinary];
    x2 = x * x; // x3 = x2*x; x4 = x3*x; x6 = x3*x3; x8 = x4*x4;
    y = collisionGeometry->y[iBinary];
    y2 = y * y; // y3 = y2*y; y4 = y3*y; y6 = y3*y3; y8 = y4*y4;
    xy = x * y;
    h_xyNNIntVsB->Fill(impactPar, x, y, weight);
    counts += weight;
    momX += weight * x;
    momX2 += weight * x2; // momX3 += weight*x3; momX4 += weight*x4; momX6 += weight*x6; momX8 += weight*x8;
    momY += weight * y;
    momY2 += weight * y2; // momY3 += weight*y3; momY4 += weight*y4; momY6 += weight*y6; momY8 += weight*y8;
    momXY += weight * xy;
  }

  momX /= counts;
  momX2 /= counts; // momX3 /= counts; momX4 /= counts; momX6 /= counts; momX8 /= counts;
  momY /= counts;
  momY2 /= counts; // momY3 /= counts; momY4 /= counts; momY6 /= counts; momY8 /= counts;
  momXY /= counts;
...
{
  double v, v2, rms, erms, omega, eomega, R2, eR2;
