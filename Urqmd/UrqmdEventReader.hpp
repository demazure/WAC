...
class UrqmdEventReader : public Task
{
 public:
  ...
  {
    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::CTOR(...) No ops" << endl;
  }
  ...
  {
    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::initialize() Started" << endl;
    ...
    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::initialize() Completed" << endl;
  }
  ...
  {
    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::reset() Started" << endl;
    event->reset();
    particleFactory->reset();
    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::reset() Completed" << endl;
  }
  ...
  {
    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::generate() Started" << endl;
    ...
    while (seekingEvent) {
      ...
      if (anEvent->GetImpactParameter() > generatorConfiguration->impactParameterMinimum &&
          anEvent->GetImpactParameter() <= generatorConfiguration->impactParameterMaximum)
        seekingEvent = false;
    }

    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::generate() new event b: " << anEvent->GetImpactParameter() << ",   ntracks: " << anEvent->GetNtrack() << endl;
    ...
    for (int iPart1 = 0; iPart1 < anEvent->GetNtrack(); iPart1++) {
      ...
      if (aParticle.pt < generatorConfiguration->ptMinimum || aParticle.pt > generatorConfiguration->ptMaximum)
        continue;
      if (aParticle.y < generatorConfiguration->yMinimum || aParticle.y > generatorConfiguration->yMaximum)
        continue;
      if (aParticle.eta < generatorConfiguration->etaMinimum || aParticle.eta > generatorConfiguration->etaMaximum)
        continue;
      if (!acceptPID(aParticle))
        continue;
    ...
    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::generate() No of accepted Particles: " << particleAccepted << endl;
    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::generate() No of rejected leptons: " << nleptons << endl;
    // event->printProperties(cout);
    if (debugLevel > 0)
      cout << "-DEBUG- UrqmdEventReader::generate() event completed!" << endl;
    return event;
  }
